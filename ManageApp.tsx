...
const sourceReports = {
  ...
  classes: 'https://courses.cs.ox.ac.uk/blocks/configurable_reports/viewreport.php?id=254&courseid=743&download=1&format=csv',
...
export const ManageApp: React.FC = () => {
  ...
  const copySourceReportLink = async (label: string, url: string) => {
    try {
      ...
      setCopiedSourceReport(label);
      setStatus(`Copied the ${label} report link. Paste it into a new browser tab.`, 'success');
      window.setTimeout(() => setCopiedSourceReport((current) => (current === label ? null : current)), 2000);
    } catch (error) {
      console.error(error);
      setStatus('Could not copy the report link. Try selecting and copying it manually.', 'error');
    }
  };
        ...
        <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div className="flex flex-col gap-4">
              <DepartmentBrand className="w-fit" imageClassName="h-16 md:h-20" />
              <div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/50">
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900/70">
                <h3 className="text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Where to get the source files</h3>
                <ul className="mt-3 space-y-3 text-sm text-slate-600 dark:text-slate-300">
                  <li>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Cohort Rules:</span>{' '}
                    Upload CSV file generated by{' '}
                    <button
                      type="button"
                      onClick={() => copySourceReportLink('cohort rules', sourceReports.cohortRules)}
                      className="font-semibold text-[#002147] underline decoration-slate-300 underline-offset-2 hover:text-[#003c7d] dark:text-indigo-300 dark:decoration-slate-600"
                    >
                      {copiedSourceReport === 'cohort rules' ? 'copied' : 'copy this report link'}
                    </button>
                    .
                  </li>
                  <li>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Lectures:</span>{' '}
                    Upload the ICS files that were uploaded to Moodle.
                  </li>
                  <li>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Classes:</span>{' '}
                    Upload CSV file generated by{' '}
                    <button
                      type="button"
                      onClick={() => copySourceReportLink('classes', sourceReports.classes)}
                      className="font-semibold text-[#002147] underline decoration-slate-300 underline-offset-2 hover:text-[#003c7d] dark:text-indigo-300 dark:decoration-slate-600"
                    >
                      {copiedSourceReport === 'classes' ? 'copied' : 'copy this report link'}
                    </button>
                    .
                  </li>
                  <li>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Practicals:</span>{' '}
                    Upload CSV file generated by{' '}
                    <button
                      type="button"
                      onClick={() => copySourceReportLink('practicals', sourceReports.practicals)}
                      className="font-semibold text-[#002147] underline decoration-slate-300 underline-offset-2 hover:text-[#003c7d] dark:text-indigo-300 dark:decoration-slate-600"
                    >
                      {copiedSourceReport === 'practicals' ? 'copied' : 'copy this report link'}
                    </button>
                    .
                  </li>
                </ul>
